...
const InteractiveQuestionnaire = ({ onComplete, initialProfile = {} }) => {
  const [currentStep, setCurrentStep] = useState(0);
  const [profile, setProfile] = useState({
    age: initialProfile.age || '',
    ageGroup: initialProfile.ageGroup || '',
    income: initialProfile.income || '',
    incomeRange: initialProfile.incomeRange || '',
    ...
    monthlySavings: initialProfile.monthlySavings || '',
    monthlySavingsRange: initialProfile.monthlySavingsRange || '',
  ...
  const steps = [
    ...
    },
    {
      ...
      subtitle: 'What amount can you realistically set aside each month?',
      type: 'monthlySavingsRange'
    },
    {
      id: 'goal',
  ...
  const monthlySavingsRanges = [
    { value: '0-200', label: '0 - 200', description: 'Just starting', savings: 100 },
    { value: '200-500', label: '200 - 500', description: 'Building habits', savings: 350 },
    { value: '500-1000', label: '500 - 1,000', description: 'Good progress', savings: 750 },
    { value: '1000-2000', label: '1,000 - 2,000', description: 'Strong saver', savings: 1500 },
    { value: '2000-5000', label: '2,000 - 5,000', description: 'Excellent saver', savings: 3500 },
    { value: '5000+', label: '5,000+', description: 'High earner', savings: 7500 }
  ];
  ...
  const handleNext = () => {
    if (currentStep < steps.length - 1) {
      ...
      const finalProfile = {
        ...
        monthlySavings: parseInt(profile.monthlySavings) || (monthlySavingsRanges.find(r => r.value === profile.monthlySavingsRange)?.savings || 500),
  ...
  const handleAutoAdvance = (field, value) => {
  ...
  const isStepComplete = () => {
    ...
    switch (step.type) {
      ...
      case 'monthlySavingsRange':
        return profile.monthlySavingsRange;
      case 'input':
  ...
  const renderStep = () => {
    ...
    switch (step.type) {
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {ageGroups.map((group) => (
              <button
                key={group.value}
                onClick={() => {
                  handleAutoAdvance('ageGroup', group.value);
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {incomeRanges.map((range) => (
              <button
                key={range.value}
                onClick={() => {
                  handleAutoAdvance('incomeRange', range.value);
      ...
      case 'monthlySavingsRange':
        return (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {monthlySavingsRanges.map((range) => (
              <button
                key={range.value}
                onClick={() => {
                  handleAutoAdvance('monthlySavingsRange', range.value);
                  updateProfile('monthlySavings', range.savings);
                }}
                className={`p-6 rounded-lg border-2 text-left transition-all hover:shadow-md ${
                  profile.monthlySavingsRange === range.value
                    ? 'border-blue-500 bg-blue-50'
                    : 'border-gray-200 hover:border-gray-300'
                }`}
              >
        ...
        return (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {goals.map((goal) => (
              <button
                key={goal.value}
                onClick={() => handleAutoAdvance('investmentGoal', goal.value)}
                className={`p-6 rounded-lg border-2 text-left transition-all hover:shadow-md ${
        ...
        return (
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {timelines.map((timeline) => (
              <button
                key={timeline.value}
                onClick={() => handleAutoAdvance('timeHorizon', timeline.value)}
        ...
        return (
          <div className="space-y-4">
            {riskLevels.map((risk) => (
              <button
                key={risk.value}
                onClick={() => handleAutoAdvance('riskTolerance', risk.value)}
  ...
  return (
    <div className="max-w-4xl mx-auto p-6">
      ...
      <div className="flex justify-between">
        ...
        {/* Only show Next button for input steps */}
        {currentStepData.type === 'input' || currentStepData.type === 'benefits' ? (
          <button
            onClick={handleNext}
